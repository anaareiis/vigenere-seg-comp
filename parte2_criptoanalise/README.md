...
A estimativa é feita utilizando o Método do Índice de Coincidência (IC).
...
- Calcula o IC de cada grupo utilizando a fórmula matemática de combinações:

$$
IC = \frac{\sum_{i=A}^{Z} f_i(f_i - 1)}{N(N - 1)}
$$

- Tira a média dos ICs válidos de todos os grupos.

Os candidatos são ordenados pelo maior IC médio, sem comparação com um
valor-alvo fixo do idioma. Quando dois valores diferem por menos de `0,003`,
o menor tamanho é priorizado para reduzir falsos positivos causados por
múltiplos do período real.
...
- Tratamento de Exceções Silenciosas (Divisão por Zero): Caso o texto fornecido seja muito curto e resulte em grupos de tamanho 1 ou menor, o cálculo daquele grupo é ignorado. Isso evita divisões por zero, que corromperiam o IC com valores nulos.
- Contagem de Grupos Válidos: A média final do IC é dividida apenas pelo número de grupos que efetivamente contribuíram para o cálculo, garantindo a precisão estatística da média.
- Ordenação Inteligente e Tolerância: A função retorna um vetor ordenado (vector<int>) do tamanho mais provável para o menos provável. Na etapa de ordenação, aplica-se uma margem de tolerância estatística: se dois tamanhos de chave apresentam ICs muito semelhantes, o algoritmo prioriza o menor tamanho. Isso mitiga "falsos positivos" gerados por pequenos textos e múltiplos da chave real.
