...
de frequência. O pipeline integrado foi validado com dados de teste;
no experimento controlado, recuperou corretamente o tamanho 9, a
chave SEGURANCA e o texto claro completo.
\end{abstract}
...
\item \textbf{Recuperação de cada letra da chave}: com o tamanho $k$
  ...
  correlação é adotado como candidato à letra da chave.
...
\begin{table}[H]
\caption{Resultado do experimento controlado}
\label{tab:resultados}
\centering
\begin{tabular}{|l|c|l|c|}
\hline
\textbf{Teste} & \textbf{Tam.} & \textbf{Chave recuperada} & \textbf{Correto?} \\
\hline
Demonstração PT & 9 & SEGURANCA & Sim \\
\hline
\end{tabular}
\end{table}
...
próximos. O pipeline preserva vários candidatos para inspeção, pois
textos curtos e tamanhos incorretos podem prejudicar a análise de
frequência. O candidato de tamanho 18 confirmou que múltiplos do
período fundamental também podem produzir uma decifração correta;
nesse caso, escolhe-se a menor chave equivalente.

\section{Conclusões}
\label{sec:conclusoes}

Os resultados demonstram que um espaço de chaves grande
($26^L$) não é
...
explora. Os testes confirmam a correção da cifração e decifração e
mostram que o pipeline integrado recupera tamanho, chave e texto claro
em dados controlados. A recuperação exata da chave \texttt{SEGURANCA}
e da mensagem confirma a integração dos módulos; entretanto, o uso de
um único texto conhecido limita a generalização do experimento.
